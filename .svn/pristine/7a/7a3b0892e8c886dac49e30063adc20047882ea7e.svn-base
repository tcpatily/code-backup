...
const currentStartingPrice = ref(0);
const discountPercentage = ref(0);
const currentStrikeoutPrice = ref(0);
const rewardPoints = ref(0);
const rewardPointsByTier = ref({});
const ltHubCode = ref('');
const hubCity = ref('');
const subType = ref('');
const productId = ref('');
...
createApp({
    setup() {
        ...
        const syncGlobalData = () => {
            ...
            // New: Log and sync price-related globals
            console.log('Global currentStartingPrice:', window.currentStartingPrice?.value);
            console.log('Global discountPercentage:', window.discountPercentage?.value);
            console.log('Global currentStrikeoutPrice:', window.currentStrikeoutPrice?.value);
            // Reward points globals
            console.log('Global rewardPoints:', window.rewardPoints?.value);
            console.log('Global rewardPointsByTier:', window.rewardPointsByTier?.value);
            ...
            // Sync price-related globals
            if (window.currentStartingPrice?.value !== undefined) {
                currentStartingPrice.value = window.currentStartingPrice.value;
            }
            if (window.discountPercentage?.value !== undefined) {
                discountPercentage.value = window.discountPercentage.value;
            }
            if (window.currentStrikeoutPrice?.value !== undefined) {
                currentStrikeoutPrice.value = window.currentStrikeoutPrice.value;
            }
            // Sync reward points globals
            if (window.rewardPoints?.value !== undefined) {
                rewardPoints.value = window.rewardPoints.value;
            }
            if (window.rewardPointsByTier?.value !== undefined) {
                rewardPointsByTier.value = window.rewardPointsByTier.value;
            }
            ...
            // New: Log price-related globals after sync
            console.log('- currentStartingPrice:', window.currentStartingPrice?.value);
            console.log('- discountPercentage:', window.discountPercentage?.value);
            console.log('- currentStrikeoutPrice:', window.currentStrikeoutPrice?.value);
            // Log reward points after sync
            console.log('- rewardPoints:', window.rewardPoints?.value);
            console.log('- rewardPointsByTier:', window.rewardPointsByTier?.value);
        };
        ...
        watch(
            ...
            (newVal) => {
                if (newVal !== undefined && !isUpdatingFromExternal) {
                    isUpdatingFromExternal = true;
                    selectedPackageClassId.value = newVal;
                    console.log('🔄 Calculate Price: selectedPackageClassId synced from global:', newVal);
                    setTimeout(() => { isUpdatingFromExternal = false; }, 10);
                }
            },
            { immediate: true }
        );
        ...
        watch(
            ...
            (newVal, oldVal) => {
                if (!isUpdatingFromExternal) {
                    console.log('🔄 Calculate Price: Local selectedPackageClassId changed:', oldVal, '→', newVal);
        ...
        // Listen for external selectedPackageClassId changes (from any source) and always sync global data
        window.addEventListener('selectedPackageClassIdChanged', (event) => {
            if (!isUpdatingFromExternal) {
                console.log('🔔 Calculate Price: Received external change event:', event.detail);
                isUpdatingFromExternal = true;
                selectedPackageClassId.value = event.detail.newValue;
                // Always sync global data to update price and other values
                syncGlobalData();
                setTimeout(() => { isUpdatingFromExternal = false; }, 10);
            }
        });
        ...
        // Room code start 
        const rooms = ref([
            { noOfAdults: 2, noOfCwb: 0, noOfCnb: 0, noOfInfant: 0 }
        ]);
        ...
        function increment(roomIndex, key) {
            ...
            const total = totalTravellers(room);
            const childInfant = childInfantCount(room);

            // Max 4 travellers per room
            if (total >= 4) return;

            // Max 3 adults per room
            if (key === "noOfAdults" && room.noOfAdults >= 3) return;

            // Child/Infant cannot exceed adults
            if (
                (key === "noOfCwb" || key === "noOfCnb" || key === "noOfInfant") &&
                (childInfant + 1) > room.noOfAdults
            ) {
                return;
            }
        ...
        function decrement(roomIndex, key) {
            const room = rooms.value[roomIndex];
            if (room[key] > (key === "noOfAdults" ? 1 : 0)) {
                room[key]--;
            }
        }

        // Helper functions for template
        function totalTravellers(room) {
            return (
                Number(room.noOfAdults) +
                Number(room.noOfCwb) +
                Number(room.noOfCnb) +
                Number(room.noOfInfant)
            );
        }
        function childInfantCount(room) {
            return (
                Number(room.noOfCwb) +
                Number(room.noOfCnb) +
                Number(room.noOfInfant)
        ...
        return {
            ...
            modifyPkgClass: () => {
                console.log('🎯 Calculate Price: modifyPkgClass called - Package class:', selectedPackageClassId.value);
                ...
                console.log('✅ Calculate Price: Package class synchronized globally');
            },
