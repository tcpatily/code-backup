...
if ($priceElement.length) {
    let price = Number(pkg.startingPrice);
    let remainder = price % 100;
    let roundedPrice = remainder >= 50 ? price + (100 - remainder) : price - remainder;
    
    $priceElement.text(`Starting From @ ₹${roundedPrice.toLocaleString('en-IN')}`);
}
