...
// Vue App
// -----------------------------------------
const isMakePaymentAvailable = ref(false);
...
createApp({
    ...
    setup() {
        ...
        const cityConfirmed = ref(false);              // must select from suggestions to confirm
        ...
        function onCityInput() {
            ...
            cityConfirmed.value = false; // typing invalidates confirmation
        ...
        function onCityBlur() {
            ...
            // Re-validate on blur so error shows if user didn't select
            validateField && validateField('city');
        }
        ...
        function selectCitySuggestion(c) {
            ...
            showCitySuggestions.value = false;
            filteredCitySuggestions.value = [];
            cityConfirmed.value = true; // confirmed only via explicit selection
            validateField && validateField('city');
        }
        ...
        function onSelectSavedContact(contactDetailsObj) {
            ...
            cityConfirmed.value = !!(contactDetailsObj.city && contactDetailsObj.city.trim());
            [
                'title', 'firstName', 'lastName', 'mobNumber', 'emailId', 'address', 'pinCode', 'stateCode', 'city'
        ...
        async function addTraveller() {
            ...
            try {
                ...
                if (tdpData.value.travellersList) {
                    pax.isTravellerSaved = true;
                }
        ...
        function getGender(title) {
            ...
            if (['Mr', 'Master'].includes(cleanTitle)) return "M";
            if (['Mrs', 'Ms', 'Miss'].includes(cleanTitle)) return "F";
            return "";
        }
        ...
        async function onAddTravellerClick(pax) {
            ...
            if (isValid) {
                // setting gender;
                pax.gender = getGender(pax.title);
                if (travellersListForDisplay.value.length == selectedTraveller.value) {
                    let payersList = travellersListForDisplay.value.filter(t => t.isPayer == "Y");
                    if (payersList.length > 0) {
                        await addTraveller();
                        goToNextTraveller();
                        isMakePaymentAvailable.value = true;
                    } else {
                        alert("Atleast one traveller should be payer");
                        isMakePaymentAvailable.value = false;
                    }
                } else {
                    await addTraveller();
                    goToNextTraveller();
            ...
            const travellersPayload = travellersListForDisplay.value
                ...
                .map(tr => {
                    ...
                    const base = {
                        ...
                        isPayer: tr.isPayer,
                        paymentPayer: 'N',
                        utilizedAmount: Number(tr.utilizedAmount) || 0,
                        isSaved: tr.usedForFutureBookings || false,
                    };
                    if (tr.isPayer) {
        ...
        function validateAllTravellers() {
            let allValid = true;
            travellersListForDisplay.value.forEach((pax) => {
                const errors = validatePaxFields(pax);
                pax.errors = { ...errors };
                // If any error field is non-empty, mark as invalid
        ...
        async function submitBooking() {
            if (!validateAllTravellers()) { // contact details & travellers
                isMakePaymentAvailable.value = false;
                return;
            }
            ...
            try {
                ...
                window.location.href = "/tcHolidays/tcPaymentGateway?tid=" + res.data.tid;
        ...
        // Allow only letters and spaces for names; sanitize on input
        function sanitizeName(val) {
            return (val || '').replace(/[^A-Za-z ]+/g, '');
        }
        function blockNameKeypress(e) {
            const k = e.key;
            // Only block printable single-character keys; allow control/navigation keys
            if (k && k.length === 1 && !/[A-Za-z ]/.test(k)) {
                e.preventDefault();
            }
        }
        function onFirstNameInput(pax) {
            const raw = pax.firstName || '';
            const sanitized = sanitizeName(raw);
            if (raw !== sanitized) pax.firstName = sanitized;
            const val = sanitized.trim();
            pax.errors.firstName = val && /^[A-Za-z ]+$/.test(val) ? '' : 'Only alphabets & spaces';
        }
        function onLastNameInput(pax) {
            const raw = pax.lastName || '';
            const sanitized = sanitizeName(raw);
            if (raw !== sanitized) pax.lastName = sanitized;
            const val = sanitized.trim();
            pax.errors.lastName = val && /^[A-Za-z ]+$/.test(val) ? '' : 'Only alphabets & spaces';
        }
        ...
        function onNameOnPanCardInput(pax) {
            const raw = pax.nameOnPanCard || '';
            const sanitized = sanitizeName(raw);
            if (raw !== sanitized) pax.nameOnPanCard = sanitized;
            const val = sanitized.trim();
            pax.errors.nameOnPanCard = val && /^[A-Za-z ]+$/.test(val) ? '' : 'Only letters & spaces';
        }
        // Backwards-compatible alias (JSP uses onPanNameInput)
        function onPanNameInput(pax) { onNameOnPanCardInput(pax); }
        ...
        function blockDigitsOnlyKeypress(e) {
            const k = e.key;
            // Only block printable single-character keys; allow control/navigation keys
        ...
        function onUtilizedAmountInput(pax) {
            const val = pax.utilizedAmount ? String(pax.utilizedAmount).replace(/[^0-9]/g, '') : '';
            if (String(pax.utilizedAmount) !== val) pax.utilizedAmount = val;
            const clean = val.trim();
            pax.errors.utilizedAmount = (clean && !isNaN(Number(clean)) && Number(clean) > 0) ? '' : 'Enter positive amount';
        }
        ...
        function validatePaxFields(pax) {
            ...
            if (isInternationalPackage.value) {
                if (!pax.passportExpireDate || !pax.passportExpireDate.trim()) {
                    errors.value.passportExpireDate = "Expiry date required";
                } else {
            ...
            if (pax.isPayer === 'Y') {
                ...
                if (pax.utilizedAmount === undefined || pax.utilizedAmount === null || String(pax.utilizedAmount).trim() === "") {
                    ...
                    if (isNaN(num) || num <= 0) {
                        errors.value.utilizedAmount = "Enter amount";
                    }
                }
            }
        ...
        function validateField(field, event) {
            // Filter keypress input early
            if (event && event.type === 'keypress') {
                const k = event.key;
                if ((field === 'firstName' || field === 'lastName' || field === 'city') && /[0-9]/.test(k)) {
                    event.preventDefault();
                    return false;
                }
                if ((field === 'mobNumber' || field === 'pinCode') && /[^0-9]/.test(k)) {
                    event.preventDefault();
                    return false;
                }
            }
            ...
            const cd = contactDetails.value || {};
            const val = (cd[field] == null ? '' : cd[field].toString()).trim();

            let msg = '';
            switch (field) {
                case 'title':
                    if (!val) msg = 'Please select a title';
                    break;
                case 'firstName':
                    if (!val) msg = 'Please enter first name';
                    else if (!/^[A-Za-z ]+$/.test(val)) msg = 'Only letters allowed';
                    break;
                case 'lastName':
                    if (!val) msg = 'Please enter last name';
                    else if (!/^[A-Za-z ]+$/.test(val)) msg = 'Only letters allowed';
                    break;
                case 'mobNumber':
                    if (!val) msg = 'Please enter mobile number';
                    else if (!/^[789]\d{9}$/.test(val)) msg = 'Enter a valid 10-digit number starting with 7, 8, or 9';
                    break;
                case 'emailId':
                    if (!val) msg = 'Please enter email';
                    else if (!/^[^\s@]+@[^\s@]+\.[A-Za-z]{2,}$/.test(val)) msg = 'Enter valid email';
                    break;
                case 'address':
                    if (!val) msg = 'Please enter address';
                    break;
                case 'pinCode':
                    if (!val) msg = 'Please enter pin code';
                    else if (!/^\d{6}$/.test(val)) msg = 'Pin code must be 6 digits';
                    break;
                case 'stateCode':
                    if (!val) msg = 'Please select state';
                    break;
                case 'city':
                    if (!val) msg = 'Please enter city';
                    else if (!cityConfirmed.value) msg = 'Please select a city from suggestions';
                    else if (!/^[A-Za-z ]+$/.test(val)) msg = 'City can only contain letters';
                    break;
                default:
                    break; // untracked field
            }

            // Only show if touched
            errors.value[field] = touchedFields.value[field] ? msg : '';

            // Recompute disabled state (simple, readable)
            const required = ['title', 'firstName', 'lastName', 'mobNumber', 'emailId', 'address', 'pinCode', 'stateCode', 'city'];
            let disable = false;
            for (let i = 0; i < required.length; i++) {
                const f = required[i];
                const raw = cd[f];
                const v = (raw == null ? '' : raw.toString()).trim();
                if (!v) { disable = true; break; }
                if ((f === 'firstName' || f === 'lastName' || f === 'city') && !/^[A-Za-z ]+$/.test(v)) { disable = true; break; }
                if (f === 'mobNumber' && !/^[789]\d{9}$/.test(v)) { disable = true; break; }
                if (f === 'emailId' && !/^[^\s@]+@[^\s@]+\.[A-Za-z]{2,}$/.test(v)) { disable = true; break; }
                if (f === 'pinCode' && !/^\d{6}$/.test(v)) { disable = true; break; }
            }
            // Additionally require city to be confirmed via suggestion
            if (!disable && !cityConfirmed.value) {
                disable = true;
            }
            if (!disable) {
                for (let i = 0; i < required.length; i++) {
                    const f = required[i];
                    if (touchedFields.value[f] && errors.value[f]) { disable = true; break; }
                }
            }
            isMakePaymentAvailable.value = !disable;
            return msg === '';
        ...
        return {
            ...
            blockNameKeypress,
            blockDigitsOnlyKeypress,
            parseDate,
            isInternationalPackage,
            gstError,
            goBack,
            isMakePaymentAvailable,
            validateField,
            touchedFields
            // getEmptyPaxErrors,
        };
