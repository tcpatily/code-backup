  ...
  <div 
    ...
    <div class="short_note">
      <span>Note:</span> The above mentioned hotels will be same or similar
          ...
          <!-- Note Block -->
         <!-- Show if notes exist for this day -->
<div class="note_blc" v-if="getNotesByDay(item.itineraryDay).length > 0">
    <span>
        <span>Note:</span> {{ getCombinedNotesByDay(item.itineraryDay) }}
    </span>
</div>
        </div>
      </div>
                                        ...
                                        <div class="tab-container tab_inclusions">
                                            <div class="tab-left">
                                                <div class="tabs">
                                                    ...
                                                    <button class="tab-btn" data-tab="transfer"  v-if="getFlagByIndex(packageDetailsResponse?.[0]?.packageDetail?.isTransferDetailsIncluded,selectedPackageClassId) === 'Y'">Transfer</button>
...
<div id="sightseeing" class="tab-details" v-if="getFlagByIndex(packageDetailsResponse?.[0]?.packageDetail?.isSightseeingDetailsIncluded, selectedPackageClassId) === 'Y'">
    <div class="tab_card">
        ...
        <div class="tabDetails_wrapper">
            <div class="card_bottom">
                <!-- Group by City -->
                <template v-for="(cityData, cityName, index) in sightseeingByCity" :key="cityName">
                    <div class="comn_blck">
                        <p class="title">{{ cityName.toUpperCase() }}</p>
                        
                        <!-- Check if multiple items or single item -->
                        <ul class="unlisted" v-if="cityData.items.length > 1">
                            <li v-for="item in cityData.items" :key="item.holidaySightseeingId">
                                {{ item.sightseeingId?.name }}
                            </li>
                        </ul>
                        
                        <!-- Single item without list -->
                        <template v-else-if="cityData.items.length === 1">
                            <span class="sub_title">{{ cityData.items[0].sightseeingId?.name }}</span>
                            <div v-if="cityData.items[0].sightseeingId?.description" 
                                 class="description" 
                                 v-html="cityData.items[0].sightseeingId?.description"></div>
                        </template>
                    </div>
                    
                    <!-- Separator between cities (except last) -->
                    <div class="seperator" v-if="index < Object.keys(sightseeingByCity).length - 1"></div>
                </template>
                
                <!-- No sightseeing message -->
                <div class="comn_blck" v-if="Object.keys(sightseeingByCity).length === 0">
                    <p class="sub_title">No sightseeing details available</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="tab_card">
        ...
        <div class="tabDetails_wrapper">
            <div class="card_bottom">
                <div class="comn_blck">
                    <div class="items_inr">
                        <!-- Meal Summary from mealCollection -->
                        <div class="item_blck" v-if="mealSummary.breakfast > 0">
                            <div class="icon">
                                <img src="/images/tcHolidays/tc-PDP/meal.svg" alt="Breakfast" />
                            </div>
                            <span class="sub_title">
                                {{ mealSummary.breakfast }} Daily breakfast
                                <template v-if="mealSummary.breakfastExcludeDays.length > 0">
                                    , except on Day {{ mealSummary.breakfastExcludeDays.join(', Day ') }}
                                </template>
                            </span>
                        </div>

                        <div class="item_blck" v-if="mealSummary.lunch > 0 || mealSummary.packedLunch > 0 || mealSummary.dinner > 0">
                            <div class="icon">
                                <img src="/images/tcHolidays/tc-PDP/meals-02.svg" alt="Meals" />
                            </div>
                            <span class="sub_title">
                                <template v-if="mealSummary.lunch > 0">{{ mealSummary.lunch }} Lunches</template>
                                <template v-if="mealSummary.packedLunch > 0">, {{ mealSummary.packedLunch }} Packed Lunches</template>
                                <template v-if="mealSummary.dinner > 0"> and {{ mealSummary.dinner }} Dinner</template>
                                as per itinerary
                            </span>
                        </div>

                        <!-- Custom meals from holidayMealOthersDetailsCollection -->
                        <div class="item_blck" v-for="meal in filteredMealCollection" :key="meal.holidayMealId">
                            <div class="icon">
                                <img 
                                    :src="meal.mealDetails && meal.mealDetails.tcilMstMealImagesCollection && meal.mealDetails.tcilMstMealImagesCollection.length > 0 
                                        ? (imageURL + holidayImageURL + 'meal/' + meal.mealDetails.tcilMstMealImagesCollection[0].imagePath) 
                                        : '/images/tcHolidays/tc-PDP/meals-02.svg'" 
                                    :alt="meal.mealDetails ? meal.mealDetails.mealName : 'Meal'" />
                            </div>
                            <span class="sub_title" v-html="meal.mealDetails ? meal.mealDetails.mealDescription : ''"></span>
                        </div>

                        <!-- Special treats section -->
                    ...
                    <!-- Special treats list -->
                    <ul class="special_treat unlisted" v-if="mealSpecialTreats.length > 0">
                                    ...
                                    <div class="tab-pane fade" id="nav-summary" role="tabpanel"
                                        ...
                                        <div class="tab-container tab_summary">
                                            <div class="tab-right summary_details">
                                                <div class="arriavl_dapt" v-if="getFlagByIndex(packageDetailsResponse?.[0]?.packageDetail?.isFlightIncluded,selectedPackageClassId) === 'Y'">
